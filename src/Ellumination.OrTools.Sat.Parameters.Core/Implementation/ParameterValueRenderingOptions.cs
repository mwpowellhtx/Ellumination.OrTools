...
namespace Ellumination.OrTools.Sat.Parameters
{
    ...
    {
        private readonly IRenderParameterValueCallbackDictionary _callbacks;
        ...
        //            : null;
        //    // TODO: TBD: we need to refactor the rendering bits to separate helper functions...
        ...
        private static IRenderParameterValueCallbackDictionary GetDefaultRenderingCallbacks()
        {
            ...
            {
                {typeof(int), x => ((int) x).RenderParameterValue()},
                {typeof(long), x => ((long) x).RenderParameterValue()},
                {typeof(bool), x => ((bool) x).RenderParameterValue()},
                {typeof(double), x => ((double) x).RenderParameterValue()}
            };
        }
        ...
        /// <summary>
        /// Simply relay <paramref name="value"/> to the extension method.
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        private static string RenderEnumParameterValue(object value) => value.RenderParameterValue();
