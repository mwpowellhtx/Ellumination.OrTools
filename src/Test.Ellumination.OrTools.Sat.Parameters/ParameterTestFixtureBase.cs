...
namespace Ellumination.OrTools.Sat.Parameters
{
    ...
    public abstract class ParameterTestFixtureBase : TestFixtureBase
    {
        ...
        {
            var original = ParameterTestCasesBase.Descriptors.AssertNotNull().AssertCollectionNotEmpty().ToArray();
            ...
            slices.AssertCollectionNotEmpty().AssertEqual(original.Length, x => x.Sum(y => y.Count()));
